...
// var destination = './wordpress/wp-content/themes/fromscratch'; // dossier à livrer
...
gulp.task('css', function () {
	gulp.src(source + "/**/*.scss")
		...
		.pipe(plugins.notify({message: 'Watch task complete'}));
});
...
gulp.task('move', function () {
	gulp.src([source + "/**/*.cur", source + "/assets/icon/*.svg"])
		.pipe(gulp.dest(destination))
		.pipe(browserSync.reload({
			stream: true
		}));
...
gulp.task('watch', ["images", "browserSync", "css", "html", "js", "move"], function () {
...
gulp.task('browserSync-orig', function () {
	browserSync.init({
		proxy: "localhost/gulproject/wordpress/"
	})
});
